...
$('#fetch-jobs').click(function(){
    $.post('/controller/updateMedias')
    .done(function() {
        $.post('/controller/files/scan');
    })
    .fail(function() {
        msgBar.warning("Warnung: Papiere konnten nicht heruntergeladen werden!", 4000);
        if(confirm("Medias could not be downloaded. Scan anyways?")){
            $.post('/controller/files/scan');
        }
    })
})

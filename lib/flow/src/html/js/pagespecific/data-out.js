...
function setWebsocketPile(status){
    $('#webservice-connection').html(status ? "ONLINE" : "OFFLINE");
    $('#webservice-pile').removeClass(status ? "offline" : "online").addClass(status ? "online" : "offline");
}

setWebsocketPile(pibConnection.getWebsocketStatus());

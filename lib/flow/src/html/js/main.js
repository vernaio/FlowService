...
var services = {
	...
	websocket : {
		...
		setStatus : function(online){
			$('#webservice-connection').html(online ? "ONLINE" : "OFFLINE");
			return;
		}
	},
	imposition : {
	...
	jdfJobTicketGenerator : {
		...
		online : false,
		response : null,
		errorMsg : 'Uuuups..! Der JDF Job Ticket Service kann nicht erreicht werden.',
		eval : function(response){
			return response.status === "online";
		},
		setStatus : function(online){
	...
	pdfJobTicketGenerator : {
		...
		online : false,
		response : null,
		errorMsg : 'Uuuups..! Der PDF Job Ticket Service kann nicht erreicht werden.',
		eval : function(response){
			return response.status === "online";
		},
		setStatus : function(online){
			...
			if(expectedServices.length > 0){
				...
				.done(
					function(response){
						...
						service.setStatus(service.online);

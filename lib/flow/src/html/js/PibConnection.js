function PibConnection(){
    ...
    function pollPibConnection(){
        ...
        .done(function(response){
            ...
            //check if anything changed in data
            serviceCache = response;
            listeners.serviceCacheWasUpdated();
    
            //check if service is not available
            if(!checkIfAllServicesOnline() && target != "services") msgBar.error('Uuuups..! Ein Dienst kann aktuell micht erreicht werden: Siehe <a href="/de/services" style="padding: 0px; text-decoration: underline;">Dienste</a>.');
        })
        .fail(function(error){

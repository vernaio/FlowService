    ...
    if(customFlowConfig.hasOwnProperty("flows")){
        for(var flowKey in customFlowConfig.flows){
            if(flowConfig.flows.hasOwnProperty(flowKey)){
                ...
                if(customFlow.hasOwnProperty("enabled") || flowKey === "binderySignatureUpload"){ //binderySignatureUpload does not need key "enabled"
                    flowConfig.flows[flowKey].enabled = (flowKey === "binderySignatureUpload" ? true : customFlow.enabled); //set value
